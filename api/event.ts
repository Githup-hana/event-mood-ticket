...
export default async function handler(req: Request) {
  ...
  // Remove /api/event prefix to get the suffix
  const pathWithoutPrefix = url.pathname.replace(/^\/api\/event/, '')
  const targetUrl = `${TARGET_BASE}${pathWithoutPrefix}${url.search}`
